...
fun get_input(day_num: Int): ArrayList<String> {
    ...
    if (!file.exists()) {
        val url = "https://adventofcode.com/2021/day/$day_num/input"
        val urlCon = URL(url).openConnection() as HttpURLConnection
        urlCon.setRequestProperty(
            "Cookie",
            "session=${File("session.txt").readText()}"
        )
        urlCon.setRequestProperty("User-Agent", "Mozilla/5.0 (X11; Linux x86_64; rv:94.0) Gecko/20100101 Firefox/94.0")
